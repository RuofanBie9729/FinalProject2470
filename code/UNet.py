...
class Model(tf.keras.Model):
    def __init__(self):
        ...
        self.encoder1 = self.block(4)
        self.encoder2 = self.block(8, max_pool=True)
        self.encoder3 = self.block(16, max_pool=True)

        self.decoder1 = self.block(32, max_pool=True, up_sample=True)
        self.decoder2 = self.block(16, up_sample=True)
        self.decoder3 = self.block(8, up_sample=True)

        self.output_prob = self.block(4, output=True)
    ...
    def block(self, filters, max_pool=False, up_sample=False, output=False):
        ...
        if output:
            net_block.add(Conv2D(3, 1, activation='softmax', padding='same'))
...
def main():
    ...
    train_inputs, train_labels = get_data('../data/train_img_r.npy', '../data/train_lab_r.npy', 'flip')
    ...
    for i in range(50):
        ...
        if (i + 1) % 10 == 0:
            show_seg(model, test_inputs[:10], test_labels[:10], 'unet_flip' + str(i + 1))
