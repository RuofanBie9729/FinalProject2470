...
class Model(tf.keras.Model):
    def __init__(self):
        ...
        self.encoder1 = self.block(32)
        self.encoder2 = self.block(64, max_pool=True)
        self.encoder3 = self.block(128, max_pool=True)

        self.decoder1 = self.block(256, max_pool=True, up_sample=True)
        self.decoder2 = self.block(128, up_sample=True)
        self.decoder3 = self.block(64, up_sample=True)

        self.output_prob = self.block(32, output=True)
...
def main():
    ...
    for i in range(50):
        ...
        if (i + 1) % 10 == 0:
            show_seg(model, test_inputs[:10], test_labels[:10], 'unet_both' + str(i + 1))
